...
public class FmOutputLayerWithAdagrad extends Layer<Feature> {
	
    private int numLabels;
    private final int SINGLE_LABEL_NUMBER = 1;
    private Matrix historyGradient;
     ...
     */
    public FmOutputLayerWithAdagrad(int dim, int numLabels, float learnRate, Vector bias, Vector regBias, Matrix weights, Vector regWeights) {
    	super(dim, learnRate, bias, regBias, weights, regWeights);
    	this.numLabels = numLabels;
    	this.historyGradient = new Matrix(weights.getRows(), weights.getColumns());
    }

    public FmOutputLayerWithAdagrad(int dim, int numLabels, int numFeatures, float learnRate, Vector regBias, Vector regWeights, Random rnd, float sdev) {  
    	super(dim, numFeatures * numLabels, learnRate, regBias, regWeights, rnd, sdev);
    	this.numLabels = numLabels;
    	this.historyGradient = new Matrix(numFeatures * numLabels, dim); 
    }
    	...
    	for(Feature feature : x.getFeatures()) {
    		...
    		for(int index = 0; index < numLabels; index++) {
	    		...
	    		for(int jj = 0; jj < dim; jj++) {
	    			...
	    			float dx = lambda * hidden.get(jj) * xi;
	    			//float g1 = lambda * hidden.get(jj) * xi;
	    			historyGradient.add(wIndex, jj, dx * dx);
	    			
	    			//cache += dx**2
	    			//x += - learning_rate * dx / (np.sqrt(cache) + eps)
	    			float updateWeight = - (float) (learnRate * dx / Math.sqrt(historyGradient.get(wIndex, jj)));
