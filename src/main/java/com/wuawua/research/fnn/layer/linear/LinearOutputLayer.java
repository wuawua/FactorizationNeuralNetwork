...
public class LinearOutputLayer extends Layer<Feature> {
     ...
     * @param regWeights
     */
    public LinearOutputLayer(int dim, int numLabels, float learnRate, Vector bias, Vector regBias, Matrix weights, Vector regWeights) {
    	super(dim, learnRate, bias, regBias, weights, regWeights);
    	this.numLabels = numLabels;
    }

    public LinearOutputLayer(int dim, int numLabels, int numFeatures, float learnRate, Vector regBias, Vector regWeights, Random rnd, float sdev) {  
    ...
    public Vector forward(DataRecord<Feature> x, Vector hidden) {
    	...
    	for(Feature feature : x.getFeatures()) {
    		...
    		for(int index = 0; index < numLabels; index++) {
    			output.add(index, bias.get(index));
    			for(int jj = 0; jj < dim; jj++) {
    				output.add(index, hidden.get(jj) * xi * weights.get(index, jj));
    			}
    		}
    	}
    ...
    public Vector backward(DataRecord<Feature> x, Vector output, Vector hidden, Vector gradient, double percent) {
    	...
    	for(Feature feature : x.getFeatures()) {
    		...
    		for(int index = 0; index < numLabels; index++) {
    			...
    			float biasUpdate = -learnRate * (lambda + regBias.get(index) * bias.get(index));
    			bias.add(index, biasUpdate);
    			
	    		for(int jj = 0; jj < dim; jj++) {
	    			nextGradient.add(jj, lambda * xi * weights.get(index, jj));
	    			float updateWeight = -(float)(learnRate * (lambda * hidden.get(jj) * xi + regWeights.get(ii) * weights.get(index,jj)));
	    			weights.add(index, jj, updateWeight);
	            }
    		}
    		featureIndex++;
    	}
